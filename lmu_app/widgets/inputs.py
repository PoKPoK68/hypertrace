...
_BARS_ONLY_W = _PAD + 3 * (_BAR_W + _BAR_GAP) - _BAR_GAP + _PAD   # 80
_BARS_X0_NO_TRACE = _PAD
_BARS_X0_TRACE    = _PAD + _TRACE_W + _TRACE_GAP
_TRACE_BASE_W     = _BARS_X0_TRACE + 3 * (_BAR_W + _BAR_GAP) - _BAR_GAP + _PAD
...
class InputsWidget(BaseWidget):
    ...
    def _base_w(self) -> int:
        return _TRACE_BASE_W if self._show_trace else _BARS_ONLY_W

    def _bar_x0(self) -> int:
        return _BARS_X0_TRACE if self._show_trace else _BARS_X0_NO_TRACE
    ...
    def on_data(self, snap: LMUSnapshot):
        ...
        # Trim samples older than the window + 1s margin
    ...
    def _draw_trace(self, p: QPainter):
        ...
        for ch, color in enumerate(_TRACE_COLORS):
