...
This stays as a thin adapter for what hasn't: `main_window.py`'s
class-based auto-preset switching and `stream/server.py`'s own per-tick
game-state checks. `DataReader.get()` builds a fresh `LMUSnapshot` from the
calc engine's shared singletons on every call. There is only one live
shared-memory connection in the app, owned by `calc/module_control.py`;
nothing here opens its own. This build has no REST integration (see
CLAUDE.md), so nothing here makes a network call either.
"""
...
# times, ...) — kept as an alias so old type hints and imports keep
# resolving; VehicleData was designed to match it field-for-field.
